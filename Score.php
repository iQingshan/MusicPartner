...
class API{
    ...
    protected function curl($url,$data=null,$cookie=false){
        ...
        if($cookie==true){
            ...
            // $res['cookie']="MUSIC_U=".$musicu[1][0].';__csrf='.$csrf[1][0];
            // $result = json_encode($res);
            // $result['cookie'] = "MUSIC_U=".$musicu[1][0].';__csrf='.$csrf[1][0];
        }else{
            $result=curl_exec($curl);
        }
        curl_close($curl);
    ...
    public function follow(){
        $url="https://music.163.com/weapi/user/follow/310290934";
        return '{"code":'.json_decode($this->curl($url,$this->prepare(array('csrf_token'=>$_COOKIE["__csrf"]))),1)["code"].'}';
    }
    //getdask 获取评分任务
    public function getdask(){
        $url = "https://mp.music.163.com/weapi/music/partner/daily/task/get";
        $res = $this->curl($url,$this->prepare(array('csrf_token'=>$_COOKIE["__csrf"])));
        //print_r(json_decode($res,1));
        $follow = $this->follow();
        $res =  $this->evaluate(json_decode($res,1));
        echo $res;
    }
    //打卡评分
    public function evaluate($data){
        $url = "https://mp.music.163.com/weapi/music/partner/work/evaluate";
	    ...
	    sleep(3);
            ...
            $info['score'] = $xing;
            $info['tags'] = $xing.'-A-1';
            $info['workId'] = $work['work']['id'];
            //print_r($info);
            $res = $this->curl($url,$this->prepare($info));
            $array[$xh]['code'] = json_decode($res,1)['code'];
            $array[$xh]['id'] = $work['work']['id'];;
            $array[$xh]['message'] = json_decode($res,1)['message'];
...
$api->follow();
//test();
if($_REQUEST["do"]=="login")//登录
{
    echo $api->login($_REQUEST["user"],md5($_REQUEST["pwd"]),$_REQUEST["code"]);
}
elseif($_REQUEST["do"]=="email")//邮箱登录
{
    echo $api->loginByEmail($_REQUEST["uin"],$_REQUEST["pwd"]);
}
elseif($_REQUEST["do"]=="sign")//签到
{
    echo $api->sign();
    
}
elseif($_REQUEST["do"]=="daka")//打卡
{
    echo $api->daka_new();
    
}
elseif($_REQUEST["do"]=="check")//关注作者
{
    echo $api->follow();
    
}
elseif($_REQUEST["do"]=="dask")//音乐人打卡
{
    echo $api->getdask();
    
}
elseif($_REQUEST["do"]=="detail")//用户详情
{
    echo $api->detail($_REQUEST["uid"]);
    
}
elseif($_REQUEST["do"]=="listen")//听歌
{
